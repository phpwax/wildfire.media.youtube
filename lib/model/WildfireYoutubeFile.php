...
//generates the tag to be displayed - return generic icon if not an image
  public function render($media_item, $size, $title="preview", $class="attached_youtube", $min_size=200){
    ...
    return '<div class="youtube_media_wrapper"><a href="'.$media_item->permalink.'" class="youtube_play lightbox fancybox fancybox.iframe"></a><img width="'.$size.'" src="http://i.ytimg.com/vi/'.$media_item->source.'/hqdefault.jpg" alt="'.$title.'" class="'.$class.'" '.$w_h.' data-src="'.$url.'?rel=0&wmode=opaque'.(($size < $min_size) ? "&controls=0": "").'" data-frameborder="0" data-allowfullscreen="true"></div>';
  }
